...
要求：
1. 必须使用中文输出，即使输入描述、README 或已有分析是英文，也要改写为中文技术情报表述。
2. 不要直接复制英文原文，不要保留英文宣传句。
3. 不要夸大，不要仅根据 star 数判断价值。
4. 如果信息不足，请用中文说明“信息不足，建议复核 README / issues / 源码”。"""

OVERVIEW_ENRICH_PROMPT = """你是企业 AI 工程技术情报日报主编。请基于输入的主区项目，生成中文编辑判断。
要求：
1. editorial_judgement 最多 3 条，每条 60-120 字。
2. 不要泛泛而谈，不要写成统计复述，不要出现 ai_agent(118) 这类计数格式。
3. 结合 Coding Agent / MCP / RAG / LLM Infra / 企业落地方向给出判断。
4. 对低成熟项目给出保守提醒。
5. 只输出 JSON：{"editorial_judgement":["...","...","..."],"main_trend":"","risk_note":"","suggested_focus":""}"""
...
def enrich_report_overview(report: dict[str, Any], client: LLMClient) -> dict[str, Any]:
    ...
    if isinstance(payload, dict):
        judgements = payload.get("editorial_judgement") or payload.get("editorial_judgements")
    ...
    for section in ("breakout", "deep_research", "long_term"):
        for item in report.get("sections", {}).get(section, []):
            if len(items) >= max_items:
                return items
            if _needs_report_enrichment(item):
                items.append(item)
    return items
...
def _has_chinese_report_fields(item: dict[str, Any]) -> bool:
    ...
    risk_text = " ".join(str(risk) for risk in risks) if isinstance(risks, list) else str(risks or "")
    return all(_is_chinese_enough(field) for field in fields) and (
        not risk_text.strip() or _is_chinese_enough(risk_text)
    )
...
def _is_chinese_enough(text: str, *, min_cjk_chars: int = 6, min_cjk_ratio: float = 0.18) -> bool:
    ...
    if cjk >= min_cjk_chars and (letters == 0 or cjk / letters >= min_cjk_ratio):
        return True
    return False

...
```bash
python -m github_ai_trend_radar.main deep-research --repo owner/repo --open
python -m github_ai_trend_radar.main deep-research --from-inbox --limit 3
```

deep-research 默认只在本地执行，不接入 GitHub Actions schedule。第一版会生成结构化资料汇总；后续可以加入更完整的 LLM 深研报告。
